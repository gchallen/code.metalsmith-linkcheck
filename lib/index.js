...
module.exports = function (config) {
  ...
  return function(files, metalsmith, done) {
    ...
    config = linkcheckDefaults.processConfig(metalsmith.source(), config);
    ...
    async.series([
        ...
        function (callback) {
          if (internal_failed.length > 0) {
            ...
            jsonfile.writeFileSync(config.failFile, internal_failed);
          }
          callback();
        },
        function (callback) {
          ...
          async.mapLimit(external, 8, function (link, finished) {
            ...
            var failed = _.union(internal_failed, external_failed);
            if (failed.length > 0) {
