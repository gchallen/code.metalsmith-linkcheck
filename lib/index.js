...
function linkcheck(config) {
  ...
  if (config.ignoreFile === undefined) {
    config.ignoreFile = 'links_ignore.json';
  }
  if (config.failFile === undefined) {
    config.failFile = 'links_failed.json';
  }
  ...
  return function(files, metalsmith, done) {
    ...
    var checked_file = path.join(metalsmith.source(), config.checkFile),
        ignore_file = path.join(metalsmith.source(), config.ignoreFile),
        fail_file = path.join(metalsmith.source(), config.failFile);
    ...
    async.series([
        ...
        function (callback) {
          ...
          async.mapLimit(external, 8, function (link, finished) {
            ...
            jsonfile.writeFileSync(fail_file, _.union(internal_failed, external_failed));
            callback();
          });
       }
    ],
